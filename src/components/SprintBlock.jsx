...
function SprintBlock({ sprint = {}, onEdit, onDelete, onView, total_task, completed_task }) {
    ...
    const getStatusBadgeColor = () => {
        switch (isActive) {
            case true: return 'bg-primary/10 text-primary border border-primary/25'
            case false: return 'bg-muted text-muted-foreground border border-border'
        ...
        <div className='w-full bg-card border border-border rounded-lg shadow-sm hover:shadow-md transition-shadow p-4 mb-3'>
            <div className="grid grid-cols-4 gap-4  items-center">
                ...
                <div className="flex gap-2 justify-end lg:mr-10">
                    ...
                    <button className="text-primary hover:opacity-80"  onClick = {() => onView(_id)} title="View">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    </button>
                    )}
                    {onEdit && (
                        <button className="text-[#00d4ff] hover:opacity-80" onClick={() => onEdit(_id)} title="Edit">
                    ...
                    {onDelete && (
                        ...
                        <button className="text-destructive hover:opacity-80" onClick={() => onDelete(_id)} title="Delete">
